...
class ExportAsyncScheduleGroup(models.Model):
    ...
    _order = "next_execution, display_name"
    ...
    @api.depends("company_id.name", "export_ids.display_name")
    def _compute_display_name(self):
        for record in self:
            exports = ", ".join(
                record.export_ids.mapped("display_name")[:MAX_EXPORTS_IN_NAME]
            )
            if len(record.export_ids) > MAX_EXPORTS_IN_NAME:
                exports += ", ..."
            record.display_name = f"{record.company_id.name}: {exports}"
    ...
    def write(self, vals):
        ...
        # Propagate scheduling fields to exports when group is updated
        propagate_fields = {
            "active",
            "user_ids",
            "next_execution",
        ...
        if any(field in vals for field in propagate_fields):
            for record in self:
                export_vals = {}
                for field in propagate_fields:
                    if field in vals:
                        export_vals[field] = vals[field]
                if export_vals:
                    record.export_ids.write(export_vals)
        return result
