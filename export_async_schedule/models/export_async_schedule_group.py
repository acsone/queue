            ...
            if record.mail_template_id and record.mail_template_id.partner_to:
                ...
                try:
                    ...
                    _logger.debug(
                        ...
                        record.mail_template_id.id,
                        e,
                    )

            record.template_partner_ids = partners
            # Auto-populate user_ids from template partners if not already set
            if users and not record.user_ids:
                record.user_ids = users

    @api.onchange("mail_template_id")
    def _onchange_mail_template_id(self):
        # Trigger recomputation of template fields when template changes
        self._compute_from_template()
