...
class ExportAsyncScheduleGroup(models.Model):
    ...
    def _compute_template_partner_ids(self):
        for record in self:
            ...
            if record.mail_template_id and record.mail_template_id.partner_to:
                # partner_to contains comma-separated partner IDs
                try:
                    partner_ids = [
                        int(pid.strip())
                        for pid in record.mail_template_id.partner_to.split(",")
                    ...
                    partners = self.env["res.partner"]
            record.template_partner_ids = partners
    ...
    def _onchange_mail_template_id(self):
        if self.mail_template_id and self.mail_template_id.partner_to:
            try:
                # Parse partner_to field (comma-separated partner IDs)
                partner_ids = [
                    int(pid.strip())
                ...
                if partner_ids:
                    partners = self.env["res.partner"].browse(partner_ids)
                    # Get users linked to these partners
