...
class ExportAsyncScheduleGroup(models.Model):
    ...
    user_ids = fields.Many2many(
        ...
        compute="_compute_user_ids_from_template",
        readonly=False,
        store=True,
        tracking=True,
    )
    ...
    template_partner_ids = fields.Many2many(
        comodel_name="res.partner",
        compute="_compute_template_partner_ids",
        string="Template Partners",
        help="Partners configured as recipients in the selected email template.",
    )

    display_name = fields.Char(compute="_compute_display_name", store=True, copy=False)

    @api.depends("mail_template_id.partner_to")
    def _compute_user_ids_from_template(self):
        """Compute user_ids from template partners (stored field)."""
        for record in self:
            partners = record._get_template_partners()
    ...
    @api.depends("mail_template_id.partner_to")
    def _compute_template_partner_ids(self):
        """Compute template_partner_ids from template partner_to field (non-stored)."""
        for record in self:
            record.template_partner_ids = record._get_template_partners()

    def _get_template_partners(self):
        """Extract partners from template partner_to field."""
        partners = self.env["res.partner"]
        if self.mail_template_id and self.mail_template_id.partner_to:
            try:
                partner_ids = [
                    int(pid.strip())
                    for pid in self.mail_template_id.partner_to.split(",")
                ...
                _logger.debug(
                    "Invalid partner_to format in template %s: %s",
    ...
    def _get_recipient_emails(self):
        # Get emails from group recipients
        group_emails = super()._get_recipient_emails()

        # Add template email_to if configured
        template_emails = ""
        if self.mail_template_id.email_to:
            template_emails = self.mail_template_id.email_to

        # Combine and deduplicate emails
        all_emails = set()
        if group_emails:
            all_emails.update(email.strip() for email in group_emails.split(","))
        if template_emails:
            all_emails.update(email.strip() for email in template_emails.split(","))

        return ",".join(sorted(all_emails))
