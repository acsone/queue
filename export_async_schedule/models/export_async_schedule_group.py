...
class ExportAsyncScheduleGroup(models.Model):
    ...
    @api.depends("mail_template_id.partner_ids")
    def _compute_template_partner_ids(self):
        for record in self:
            record.template_partner_ids = record.mail_template_id.partner_ids
    ...
    def _onchange_mail_template_id(self):
        if self.mail_template_id and self.mail_template_id.partner_ids:
            # Get users from template partners
            partners = self.mail_template_id.partner_ids
